...
class IndexController extends Controller{
    public function index(){
        $left_array  = $this->get_left_info();   //左侧高级分类和中级分类      
        $right_array = $this->get_right_info();    //右侧中级分类和数据信息      
        $hot_array = $this->get_hot_info();   //热门网站     
        $ad_array  = $this->get_hot_info($type = 1);   //广告信息    
        $pic_array = $this->get_hot_info($type = 2);  //图片区     
        $bottom_array = $this->get_bottom_info();  //底部高级分类和数据信息
    ...
    public function more(){
        $map['middle_id'] = I('middle_id',0,'int');  //接收中级分类id
        $elementary_level = M('elementary_level')->where($map)->select();   //查找初级分类
        foreach($elementary_level as $elementary){           
            $array = M('datalist')->where(array('elementary_id'=>$elementary['id']))->select();  //初级分类数据          
            $count = M('datalist')->where(array('elementary_id'=>$elementary['id']))->count();  //初级分类下的数量       
            $key = $elementary['elementary_name'].'('.$count.')';   //组装成数组
            $datalist[$key] = $array;
        }
        $more_info = $datalist;
